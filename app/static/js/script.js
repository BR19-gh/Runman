...
function firstTimeOpened(answer) {
    localStorage.setItem("firstLoad", answer)
}
let dontPressAandS = 0;

function showEnterNNPopup() {
    if ((localStorage.getItem("firstLoad")) == null || (localStorage.getItem("firstLoad")) == 'no') {
        document.getElementById("enterNNPopup").style.display = "block";
        dontPressAandS = 1;
        if (localStorage.nickname != undefined || localStorage.nickname != null)
            firstTimeOpened('no')
...
document.getElementById("English").addEventListener("click", () => {
    playOrStop(menuS, VoiceOn);
    playOrStop(menuS, VoiceOn);
    window.location.assign("app/templates/index.html");
});

document.getElementById("Arabic").addEventListener("click", () => {
    playOrStop(menuS, VoiceOn);
    playOrStop(menuS, VoiceOn);
    window.location.assign("app/templates/indexAr.html");
});
